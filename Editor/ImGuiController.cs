...
namespace ImGuiNET
{
    ...
    public class ImGuiController : IDisposable
    {
        ...
        private const string ImGuiVertexShaderSource = @"#version 450
...
";
        ...
        private const string ImGuiFragmentShaderSource = @"#version 450
...
void main()
{
    vec4 tex = texture(sampler2D(mainTexture, mainSampler), fs_texCoord);
    out_color = fs_color * tex;
}
";
        ...
        public void CreateDeviceResources(GraphicsDevice gd, OutputDescription outputDescription)
        {
            ...
            ShaderDescription vertexShaderDesc = new ShaderDescription(
                ShaderStages.Vertex,
                Encoding.UTF8.GetBytes(ImGuiVertexShaderSource),
                "main");

            ShaderDescription fragmentShaderDesc = new ShaderDescription(
                ShaderStages.Fragment,
                Encoding.UTF8.GetBytes(ImGuiFragmentShaderSource),
                "main");
        ...
        private void UpdateImGuiInput(InputSnapshot snapshot)
        {
            ...
            _controlDown = _pressedKeys.Contains(Key.ControlLeft) || _pressedKeys.Contains(Key.ControlRight);
            _shiftDown = _pressedKeys.Contains(Key.ShiftLeft) || _pressedKeys.Contains(Key.ShiftRight);
            _altDown = _pressedKeys.Contains(Key.AltLeft) || _pressedKeys.Contains(Key.AltRight);
            _winKeyDown = _pressedKeys.Contains(Key.WinLeft) || _pressedKeys.Contains(Key.WinRight);

            io.AddKeyEvent(ImGuiKey.ModCtrl, _controlDown);
            io.AddKeyEvent(ImGuiKey.ModShift, _shiftDown);
            io.AddKeyEvent(ImGuiKey.ModAlt, _altDown);
            io.AddKeyEvent(ImGuiKey.ModSuper, _winKeyDown);
        }
